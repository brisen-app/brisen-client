...
export function drawCard(c: AppContextType): PlayedCard | undefined {
  const card = selectCard(c) ?? undefined
  if (!card) return card

  const playerList = getParentPlayerList(card, c) ?? [...shuffled(c.players)].sort((a, b) => a.playCount - b.playCount)

  return toPlayedCard(card, playerList, c)
}

/**
 * Retrieves the next card to be played based on the given app context.
 */
function selectCard(c: AppContextType): Card | null | undefined {
  console.debug('Drawing card...')

  const { orderedCandidates, unorderedCandidates } = findCandidates(c)

  const startingCards = getStartingCards(orderedCandidates)
  if (startingCards.size > 0) return getRandom(startingCards.values())

  const nextCards = getNextCards(orderedCandidates, c)
  if (nextCards.size > 0) return getRandom(nextCards.values())

  const endingCards = getEndingCards(orderedCandidates, unorderedCandidates, c)
  return getRandom([...unorderedCandidates.values(), ...endingCards.values()])
}

/**
 * Finds all valid candidates to be played based on the given app context.
 */
function findCandidates(c: AppContextType) {
  ...
  for (const packId of c.playlist) {
    ...
    for (const cardId of pack.cards) {
      ...
      if (!card || !isCandidate(card, c)) continue
...
export function getPlayableCards(packId: string, c: AppContextType) {
  const { orderedCandidates, unorderedCandidates } = findCandidates({
    ...c,
    playedCards: [],
    playedIds: new Set(),
    playlist: [packId],
 ...
 */
function isCandidate(card: Card, c: AppContextType) {
  ...
  const allParentsHaveBeenPlayed = [...CardRelationManager.getParents(card.id)].every(parent => c.playedIds.has(parent))
...
function getStartingCards(orderedCandidates: Map<string, Card>): Map<string, Card> {
  return new Map([...orderedCandidates].filter(([_, v]) => v.order === 'starting'))
}

function getEndingCards(
  ...
  for (const packId of c.playlist) {
    ...
    if (candidatesFromPack.some(c => c.order !== 'ending')) continue
    availableEndingCards.forEach(c => availableEndingCards.set(c.id, c))
  }
 ...
 */
function getNextCards(orderedCandidates: Map<string, Card>, c: AppContextType) {
  return new Map(
...
function toPlayedCard(card: Card, players: Player[], c: AppContextType): PlayedCard {
  const { formattedContent, featuredPlayers } = formatContent(card, players)

  return {
    ...card,
    minPlayers: CardManager.getRequiredPlayerCount(card),
    pack: getClosestPack(card, c),
    players,
...
function getClosestPack(card: Card, c: AppContextType): Pack | null {
  for (const packId of c.playlist) {
  ...
  while (parent !== null) {
    parent = CardRelationManager.getPlayedParent(card.id, c.playedIds)
    if (parent) {
      const pack = c.playedCards.find(c => c.id === parent)?.pack ?? null
      if (pack) return pack
    }
  }
 ...
 */
function getParentPlayerList(card: Card, c: AppContextType) {
