...
import PackFeaturedView from "./PackFeaturedView";
...
export default function GridContainer(props: Readonly<{
    ...
    function itemView(item: SupabaseEntity) {
        if (PackManager.isPack(item)) return style == 'list' ?
    ...
    return (
        <FlatList
            ...
            renderItem={({ item: items }) => <View style={{ width: itemWidth, gap: 8 }}>
                {items.map(( item ) => (
                    itemView(item)
